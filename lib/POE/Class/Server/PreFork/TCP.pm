package POE::Class::Server::PreFork::TCP;
...
sub create_states {
    ...
    $poe_kernel->state(int => $self, "signal_int");
...
POE::Class::Server::PreFork::TCP - Subclass of POE::Class::Server::TCP which
...
=head1 SYNOPSIS
    ...
    @My::Server::ISA = qw(POE::Class::Conn::TCPStream);
    ...
    my $server = new POE::Class::Server::PreFork::TCP(
        ...
        # Same options as POE::Class::Server::TCP
        port         => 8080,
        conn_class   => 'My::Conn',
    );
...
POE::Class::Server::PreFork::TCP is a subclass of L<POE::Class::Server::TCP>.
...
You should see L<POE::Class::Server::TCP> for details on the inherited
interface.
...
handler PreFork::TCP resumes acception on this wheel and sets the alarm used to
...
Overrides the connection handler in L<POE::Class::Server::TCP>, returns if the
...
This handler also checks for C<max_requests()>. If PreFork::TCP reaches
...
In the C<create_states()> method PreFork::TCP creates a state called int with a
...
Overrides the C<connect()> method in L<POE::Class::Server::TCP>. Calles
